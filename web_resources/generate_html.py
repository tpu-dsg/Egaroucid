...
GUI_VERSION_DOT = '7.7.0'
GUI_DATE_STR = '2025/08/16'

CONSOLE_VERSION_DOT = '7.7.0'
CONSOLE_DATE_STR = '2025/08/16'
...
with open('main_page_url.txt', 'r', encoding='utf-8') as f:
    main_page_url = f.readline().strip()
...
def generate_language(elements_dir):
    lang_root = os.path.normpath(elements_dir).strip()
    lang_root = lang_root.replace('\\', '/')
    if not lang_root:
        raise ValueError('Empty language directory specified.')
    if not os.path.isdir(lang_root):
        raise FileNotFoundError(f"Language directory '{lang_root}' does not exist.")

    generated_root = 'generated/' + lang_root
    if os.path.exists(generated_root):
        shutil.rmtree(generated_root)
    os.makedirs(generated_root, exist_ok=True)

    css_file = lang_root + '/style.css'

    with open(lang_root + '/head.html', 'r', encoding='utf-8') as f:
        head = f.read()

    with open(lang_root + '/head2.html', 'r', encoding='utf-8') as f:
        head2 = f.read()

    menu = '<div class="menu_bar">\n'
    with open(lang_root + '/menu_elements.txt', encoding='utf-8') as f:
    ...
    for text, link in menu_elems:
        if link[0] == '/':
            full_link = main_page_url + lang_root + link
            menu += '<div class="menu_button"><a class="menu_a" href="' + full_link + '">' + text + '</a></div>'
        else:
            menu += '<div class="menu_button"><a class="menu_a" href="' + link + '" target="_blank" el=”noopener noreferrer”>' + text + '</div></a>\n'
    menu += '</div>\n'

    with open(lang_root + '/tweet.html', 'r', encoding='utf-8') as f:
        tweet = f.read()

    with open(lang_root + '/foot.html', 'r', encoding='utf-8') as f:
        foot = f.read()

    with open(lang_root + '/main_page_title.txt', 'r', encoding='utf-8') as f:
        main_page_title = f.read()

    with open(lang_root + '/meta_description.txt', 'r', encoding='utf-8') as f:
        meta_description = f.readline()

    with open(lang_root + '/release_gui.html', 'r', encoding='utf-8') as f:
    ...
    with open(lang_root + '/release_gui_source.html', 'r', encoding='utf-8') as f:
    ...
    with open(lang_root + '/release_console_zip.html', 'r', encoding='utf-8') as f:
    ...
    with open(lang_root + '/release_console_source.html', 'r', encoding='utf-8') as f:
    ...
    with open(lang_root + '/download_button.html', 'r', encoding='utf-8') as f:
    ...
    def create_html(dr):
        noenglish = os.path.exists(dr + '/noenglish.txt')
        need_tex_js = os.path.exists(dr + '/need_tex_js.txt')
        alternate = ''
        lang_prefix = lang_root + '/'
        relative_dr = dr[len(lang_prefix):] if dr.startswith(lang_prefix) else ''
        if not noenglish:
            for lang in langs:
                if lang[0] == lang_root:
                    continue
                if relative_dr:
                    alternate += '<link rel="alternate" hreflang="' + lang[0] + '" href="' + main_page_url + lang[0] + '/' + relative_dr + '/"/>\n'
                else:
                    alternate += '<link rel="alternate" hreflang="' + lang[0] + '" href="' + main_page_url + lang[0] + '/"/>\n'
            if relative_dr:
                alternate += '<link rel="alternate" hreflang="x-default" href="' + main_page_url + 'en/' + relative_dr + '/">\n'
            else:
        ...
        if os.path.exists(dr + '/img'):
            img_files = glob.glob(dr + '/img/**', recursive=True)
            os.mkdir(out_dr + '/img')
    ...
    create_html(lang_root)


def main():
    targets = sys.argv[1:]
    if not targets:
        targets = [lang[0] for lang in langs]
    if not targets:
        print('No language directories specified and lang.txt is empty.', file=sys.stderr)
        sys.exit(1)
    had_error = False
    for target in targets:
        try:
            ...
            had_error = True
            print(f'Failed to generate HTML for {target}: {exc}', file=sys.stderr)
    if had_error:
        sys.exit(1)
